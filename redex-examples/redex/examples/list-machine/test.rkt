...
(test-equal (judgment-holds (var-lookup (empty x ↦ nil) x a) a)
            (list (term nil #:lang list-machine)))
...
(test-equal (judgment-holds (:lookup (x : nil (y : (list nil) empty)) y τ) τ)
            (list (term (list nil) #:lang list-machine)))

(test-equal (judgment-holds (:set empty x nil Γ) Γ)
            (list (term (x : nil empty) #:lang list-machine)))
(test-equal (judgment-holds (:set (x : nil empty) x (list nil) Γ) Γ)
            (list (term (x : (list nil) empty) #:lang list-machine)))
(test-equal (judgment-holds (:set (x : nil empty) y (list nil) Γ) Γ)
            (list (term (x : nil (y : (list nil) empty)) #:lang list-machine)))
...
(test-equal (judgment-holds (check-instr (l0 : (x : (list nil) empty) empty)
                                         ...
                                         Γ)
                            Γ)
            (list (term (x : (listcons nil) empty) #:lang list-machine)))
...
(test-equal (judgment-holds (check-instr empty
                                         ...
                                         Γ)
                            Γ)
            (list (term (x : (listcons nil) (y : nil empty)) #:lang list-machine)))
(test-equal (judgment-holds (check-instr empty
                                         (x : (listcons nil) empty)
                                         (fetch-field x 1 y)
                                         Γ)
                            Γ)
            (list (term (x : (listcons nil) (y : (list nil) empty)) #:lang list-machine)))
(test-equal (judgment-holds (check-instr empty
                                         (x : (listcons nil) 
                                            (y : (list (list nil)) 
                                               (z : nil empty)))
