...
(test (hole? (sexp->term (term hole))) #t)
(test (hole? (sexp->term (term (hole #f)))) #f)
(test (hole? (sexp->term (term (hole the-name)))) #f)

(define-syntax-rule (term-let-fn-lifted [(fn def) ...] body)
  (term-let-fn [(fn (lambda (t) (sexp->term (def (term->sexp t))))) ...]
    body))

(test (term-let-fn-lifted ((f (lambda (q) q)))
                   (term (f 1 2 3)))
      (term (1 2 3)))

(test (term-let-fn-lifted ((f (lambda (q) `(y ,(car q)))))
                   (term (f (zzzz))))
      (term (y (zzzz))))

(test (term-let-fn-lifted ((f (λ (x) (add1 (car x)))))
                   (term (f 2)))
      (term 3))

(test (term-let ([((x ...) ...) (list (list 1 1) (list 2 2) (list 3 3))])
                (term-let-fn-lifted ((f (λ (x) (car x))))
                             (term ((qq (f x) ...) ...))))
      (term ((qq 1 1) (qq 2 2) (qq 3 3))))

(test (term-let-fn-lifted ((f (lambda (x) (car x))))
                   (term (f hole)))
      (term hole))

(test (term-let-fn-lifted ((f (lambda (q) `(y ,(car q)))))
                   (term-let-fn-lifted ((g (lambda (x) `(ff ,(car x)))))
                                (term (g (f (zzzz))))))
      (term (ff (y (zzzz)))))
...
(test (term-let-fn-lifted ((metafun car))
                   (term-let ((x 'whatever)
                              ((y ...) '(4 5 6)))
                             (term (((metafun x) y) ...))))
      '((whatever 4) (whatever 5) (whatever 6)))

(test (term-let-fn-lifted ((metafun (λ (x) (car x))))
                   (term-let (((y ...) '(4 5 6)))
                             (term ((y (metafun 1)) ...))))
      '((4 1) (5 1) (6 1)))

(test (term-let-fn-lifted ((f (compose add1 car)))
                   (term-let (((x ...) '(1 2 3))
                              ((y ...) '(a b c)))
                             (term (((f x) y) ...))))
      '((2 a) (3 b) (4 c)))

(test (term-let-fn-lifted ((f (curry foldl + 0)))
                   (term-let (((x ...) '(1 2 3)))
                             (term (f x ...))))
      6)
